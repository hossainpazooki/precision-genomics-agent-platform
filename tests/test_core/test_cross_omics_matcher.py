...
class TestGeneCorrelations:
    def test_returns_dataframe(self, matcher, sample_proteomics_df, sample_rnaseq_df):
        result = matcher.compute_gene_correlations(sample_proteomics_df, sample_rnaseq_df)
        assert isinstance(result, pd.DataFrame)
        assert "gene" in result.columns
        assert "r_squared" in result.columns

    def test_shared_genes(self, matcher, sample_proteomics_df, sample_rnaseq_df):
        result = matcher.compute_gene_correlations(sample_proteomics_df, sample_rnaseq_df)
...
class TestDistanceMatrix:
    def test_shape(self, matcher, sample_proteomics_df, sample_rnaseq_df):
        shared_genes = sorted(set(sample_proteomics_df.columns) & set(sample_rnaseq_df.columns))
        dist = matcher.build_distance_matrix(sample_proteomics_df, sample_rnaseq_df, shared_genes)
        shared_samples = sorted(set(sample_proteomics_df.index) & set(sample_rnaseq_df.index))
        assert dist.shape == (len(shared_samples), len(shared_samples))

    def test_expression_rank(self, matcher, sample_proteomics_df, sample_rnaseq_df):
        shared_genes = sorted(set(sample_proteomics_df.columns) & set(sample_rnaseq_df.columns))
    ...
    def test_linear_model(self, matcher, sample_proteomics_df, sample_rnaseq_df):
        shared_genes = sorted(set(sample_proteomics_df.columns) & set(sample_rnaseq_df.columns))
        dist = matcher.build_distance_matrix(
            sample_proteomics_df,
            sample_rnaseq_df,
...
class TestIdentifyMismatches:
    def test_hungarian(self, matcher, sample_proteomics_df, sample_rnaseq_df):
        ...
        results = matcher.identify_mismatches(dist, shared_samples, n_iterations=10)
    ...
    def test_iterative(self, matcher, sample_proteomics_df, sample_rnaseq_df):
        ...
        shared_samples = sorted(set(sample_proteomics_df.index) & set(sample_rnaseq_df.index))
        dist = matcher.build_distance_matrix(sample_proteomics_df, sample_rnaseq_df, shared_genes)
        results = matcher.identify_mismatches(dist, shared_samples, n_iterations=20, sampling_fraction=0.7)
        for r in results:
    ...
    def test_no_mismatches(self, matcher):
        ...
        results = matcher.identify_mismatches(dist, samples, n_iterations=10)
