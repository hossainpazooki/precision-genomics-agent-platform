...
class TestFilterGenes:
    def test_separates_available_and_filtered(self, af, sample_proteomics_df):
        available, filtered, scores = af.filter_genes(sample_proteomics_df, threshold=0.9)
    ...
    def test_high_threshold_filters_more(self, af, sample_proteomics_df):
        avail_90, filt_90, _ = af.filter_genes(sample_proteomics_df, threshold=0.9)
        avail_50, filt_50, _ = af.filter_genes(sample_proteomics_df, threshold=0.5)
        assert len(avail_90) <= len(avail_50)

    def test_low_threshold_keeps_more(self, af, sample_proteomics_df):
        avail_10, _, _ = af.filter_genes(sample_proteomics_df, threshold=0.1)
        avail_99, _, _ = af.filter_genes(sample_proteomics_df, threshold=0.99)
        assert len(avail_10) >= len(avail_99)

    def test_all_available_genes(self, af):
        df = pd.DataFrame(np.ones((10, 5)), columns=[f"G{i}" for i in range(5)])
        available, filtered, _ = af.filter_genes(df, threshold=0.9)
        assert len(available) == 5
        assert len(filtered) == 0

    def test_no_available_genes(self, af):
        df = pd.DataFrame(np.full((10, 5), np.nan), columns=[f"G{i}" for i in range(5)])
        available, filtered, _ = af.filter_genes(df, threshold=0.9)
...
class TestComparePrePostImputation:
    def test_shows_rescued_genes(self, af, sample_proteomics_df, sample_clinical_df):
        imputer = OmicsImputer()
        imputed, _ = imputer.impute(sample_proteomics_df, sample_clinical_df)
        result = af.compare_pre_post_imputation(sample_proteomics_df, imputed, threshold=0.9)
