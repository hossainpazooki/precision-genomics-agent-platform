...
async def run_cosmo_pipeline_activity(config: dict) -> dict:
    """Run the full COSMO-inspired pipeline end-to-end."""
    from core.pipeline import COSMOInspiredPipeline

    pipeline = COSMOInspiredPipeline(
...
async def upload_model_to_gcs_activity(model_bytes_hex: str, bucket: str, path: str) -> dict:
    """Upload a serialized model artifact to GCS."""
    from core.model_registry import save_to_gcs

    artifact = bytes.fromhex(model_bytes_hex)
    uri = save_to_gcs(artifact, bucket, path)
    return {"artifact_uri": uri, "bucket": bucket, "path": path}


async def log_experiment_metrics_activity(
    project: str,
    location: str,
    experiment_name: str,
    run_name: str,
    ...
    """Log metrics and params to a Vertex AI experiment run."""
    from core.experiment_tracker import ExperimentTracker

    tracker = ExperimentTracker(
        project=project, location=location, experiment_name=experiment_name
    )
    tracker.start_run(run_name)
    tracker.log_params(params)
    tracker.log_metrics(metrics)
    tracker.end_run()
    return {"run_name": run_name, "logged_metrics": list(metrics.keys())}
